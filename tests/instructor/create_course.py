...
class JWTClient:
    ...
    def create(self, endpoint=None, data=None, files=None):
        """
        ...
        if endpoint is None or self.base_endpoint not in str(endpoint):
            endpoint = f"http://localhost:8000/api/courses/create/" 
        r = requests.post(endpoint, headers=headers, data=data, files=files)
        print(r.json())
        print(r.status_code)
        data = r.json()
        return data
...
if __name__ == "__main__":
    ...
    with open(image_path, 'rb') as file:
        files = {'image': file}
        lookup_1_data = client.create(data=data, files=files)